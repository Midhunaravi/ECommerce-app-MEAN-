...
export class ProductDetailComponent implements OnInit {
  ...
  addToCart() {
    ...
    if(this.product.stock <= 0){
      this.toastr.error('Cannot add item due to out of Stock', 'Sorry',{positionClass : "toast-top-right"});
      return;
    }

    this.cartService.addItem(newCartItem)
    this.toastr.success('Cart Item Added', 'Yeaaaaaa!!',{positionClass : "toast-top-right"});
  }
